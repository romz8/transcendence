...
class PongRemote extends HTMLElement {
    constructor() {
        super()
        this.hasRoom = false;
    ...
    async connectedCallback() {
        ...
        let joinGameButton = addGameButton("crt-game" ,"Join Game", "/waitroom/join/", this);

        function addGameButton(name, path, self){
            let GameButton = document.createElement("a");
            GameButton.className = "btn btn-primary";
            GameButton.innerText = name;
            GameButton.setAttribute("href", path);
            GameButton.type = "button";
            self.innerHTML += (GameButton);
            return GameButton;
        }
        if (this.hasRoom){
            createGameButton.className += " disabled";
            joinGameButton.className += " disabled";
            var deleteButton = addGameButton("Delete Waiting Room", "/waitroom/delete/");
            deleteButton.addEventListener('click', waitRoomView);
        }
        createGameButton.addEventListener('click', waitRoomView);
        joinGameButton.addEventListener('click', waitRoomView);
...
export default function gamerem () {
    return ('<pong-remote></pong-remote>');
}