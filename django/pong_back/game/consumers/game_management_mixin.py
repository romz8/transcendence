...
class GameManager:
    ...
    ****************************************************************************
    ...
    async def check_collision(self):
        ...
        #wall
        if  game_state['ballY'] + init['ballRadius'] >= PITCHHEIGHT + PADDING or game_state['ballY'] - init['ballRadius'] <= PADDING: 
            game_state['ballSpeedY'] *= -1
    ...
    async def check_goal(self):
        """
         ...
         """
        if  game_state['ballX'] - init['ballRadius'] < PADDING:
            #game_state['ballSpeedX'] *= -1
        ...
        if  game_state['ballX'] + init['ballRadius'] > PITCHWIDTH + PADDING:
            #game_state['ballSpeedX'] *= -1
            await self.set_game_state()
            self.room.goal = True
