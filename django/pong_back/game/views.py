...
from .serializer import MatchSerializer,  RoomSerializer, TournamentSerializer,  ParticipSerializer #, ParticipDetailSerializer ,MatchDetailSerializer, TournamentDetailSerializer
...
#         deserialized = ProfileSerializer(data=request.data)
...
#     serializer_class = MatchSerializer #in real prod use : MatchDetailSerializer

#TEST MEGA DIRTY
class MatchList(APIView):
    def get(self, request):
        return Response({"pong":"THIS WORK BITCH"}, status=status.HTTP_200_OK)
...
class SingleTournament(RetrieveAPIView):
    ...
    serializer_class = TournamentSerializer #TournamentDetailSerializer
...
class TournamentPlayerList(ListAPIView):
    ...
    '''
    serializer_class = ParticipSerializer #in real prod ParticipDetailSerializer
...
class TournamentMatchList(ListAPIView):
    serializer_class = MatchSerializer #prod MatchDetailSerializer
...
#                 user = User.objects.create(username=temp_name)  # Create a new User instance
#                 ai = Profile.objects.create(user=user, playername=temp_name, bio="", is_ai=True)
...
#         ai_list = [x.playername for x in ai_profiles]
