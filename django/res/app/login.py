...
from app.utilsName import recursiveNameGen
...
def insertLogin(request):
    if request.method == "POST":
        ...
        if response.status_code == 200:
            body = response.json()
            exist = Users.objects.filter(id=body.get('id')).exists()
            resp = {'response': 'GET'}
            if not exist:
                user = Users(id=body.get('id'), username=body.get('login'), alias=body.get('login'), img=body.get('image').get('link'),
                            intra=True, name=body.get('first_name'), lastname=body.get('last_name'),
